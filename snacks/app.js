...
function createProductCard(item, cfg) {
    const cardBg = cfg.theme.templateId === 'neon-vibes' ? 'bg-gray-800' : 'bg-white shadow-lg border border-gray-100';
...
window.openProductModal = function(itemId) {
    ...
    document.getElementById('modal-price').innerText = item.price.toFixed(2) + cfg.identity.currency;
    ...
    if (item.allergens && item.allergens.length > 0) {
        ...
        document.getElementById('modal-allergens').innerText = item.allergens.join(', ');
    } else {

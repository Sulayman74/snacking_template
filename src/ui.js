  ...
  if (loyaltyBtn) {
    if (user) {
      if (isAdmin) {
        ...
        if (loyaltyDesc) loyaltyDesc.innerText = "Scannez le QR Code d'un client pour créditer ses points.";
        loyaltyBtn.setAttribute("data-action", "open-admin-scanner");
        loyaltyBtn.innerHTML = '<i class="fas fa-camera mr-2"></i> Scanner Client';
      } else {
        if (loyaltyTitle) loyaltyTitle.innerText = cfg.loyalty?.programName || "Club Fidélité";
...
function updateCTAs(cfg) {
  ...
  if (isClickAndCollect) {
    setupCtaAction(mobileCtaBtn, mobileCtaIcon, desktopCtaBtn, "open-cart", "fas fa-shopping-bag", "Commander");
  } else if (isDelivery) {
    setupCtaAction(mobileCtaBtn, mobileCtaIcon, desktopCtaBtn, "open-delivery", "fas fa-motorcycle", "Livraison", cfg.deliveryUrl);
  } else {
    setupCtaAction(mobileCtaBtn, mobileCtaIcon, desktopCtaBtn, "call-phone", "fas fa-phone animate-pulse", cfg.contact?.phone || "Appeler", null, phoneClean);
...
function setupCtaAction(mobileBtn, mobileIcon, desktopBtn, action, iconClass, text, url = null, phone = null) {
  if (mobileBtn) {
...
function updateFooterAndStatusUI(cfg) {
  ...
  if (footerAddr && cfg.contact?.address) {
    ...
    const mapLink = a.googleMapsUrl || (isApple ? `https://maps.apple.com/?q=${encodeURIComponent(fullAddr)}` : `https://maps.google.com/?q=${encodeURIComponent(fullAddr)}`);
  ...
  if (socialsContainer && s) {
    ...
    if (s.instagram) socialsContainer.innerHTML += `<a href="${s.instagram}" target="_blank" class="hover:-translate-y-1 transition-transform"><i class="fab fa-instagram bg-linear-to-tr from-[#f09433] via-[#dc2743] to-[#bc1888] text-transparent bg-clip-text text-2xl"></i></a>`;
    if (s.facebook) socialsContainer.innerHTML += `<a href="${s.facebook}" target="_blank" class="hover:-translate-y-1 transition-transform"><i class="fab fa-facebook text-[#1877F2] text-2xl"></i></a>`;
    if (s.tiktok) socialsContainer.innerHTML += `<a href="${s.tiktok}" target="_blank" class="hover:-translate-y-1 transition-transform group"><i class="fab fa-tiktok text-white group-hover:drop-shadow-[2px_2px_0_#ff0050] transition-all text-2xl"></i></a>`;
  }
  ...
  if (hoursList && cfg.hours) {
    ...
    hoursList.innerHTML = cfg.hours.map((h, index) => {
  ...
  if (mobileBtn && mobileOverlay) {
    mobileBtn.onclick = () => {
      ...
      if (isClosed) {
        ...
        setTimeout(() => mobileOverlay.classList.add("flex", "opacity-100"), 10);
...
window.triggerVibration = (type = "light") => {
  if ("vibrate" in navigator) navigator.vibrate(type === "light" ? 40 : [100, 50, 100]);
};
